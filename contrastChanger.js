...
export default function contrastChanger() {
    ...
    if (contrastType === "Invert") {
        ...
        const textElements = document.querySelectorAll(
            "h1, h2, h3, h4, h5, h6, p, li, span, div, section"
        );
        console.log(textElements); // Check if it's empty
        applyStyles(textElements, selectedTheme.text);

        // Apply styles to links and their children
        const linksAndChildren = [];
        document.querySelectorAll("a:not(iframe)").forEach((link) => {
            linksAndChildren.push(link, ...link.querySelectorAll("*:not(iframe)"));
        });
        applyStyles(linksAndChildren, selectedTheme.link);

        // Apply styles to input and button elements
        const inputElements = document.querySelectorAll("input, button");
        applyStyles(inputElements, selectedTheme.input);
        ...
        mutations.forEach((mutation) => {
            mutation.addedNodes.forEach((node) => {
                if (node.nodeType === Node.ELEMENT_NODE && node.tagName !== "IFRAME") {
                    ...
                    const dynamicTextElements = node.querySelectorAll(
                        "h1, h2, h3, h4, h5, h6, p, li, span, div, section"
                    );
                    applyStyles(dynamicTextElements, themes[contrastType].text);
                }
            });
        });
    });
